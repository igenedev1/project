...
(function () {
    ...
    getMandaysFormat=(hour,min)=>{
        return Math.round(((((parseInt(hour)*60)+parseInt(min))/(8*60)) + Number.EPSILON) * 10000) / 10000        
    }
    ...
    addDailyDetails=(data)=>{
        $('.username-text').html(`${data.username} - ${data.date}`)
        $('#intime_field').html(data.intime);
        $('#outtime_field').html(data.outtime);
        $('#attendance_field').html(data.attendance);
        $('#total_hours_field').html(`${data.total_hours} ( ${data.total_hours!=null ? getMandaysFormat(data.total_hours.split(':')[0],data.total_hours.split(':')[1]):'None'} )`);
        ...
        data.dailies.map(work => {
            ...
            if(work.slottype==="PROD_SHOT"||work.slottype==="TEST_SHOT"){
                cel1=$(`<td> <span class="slot-main slot-type">${slotdata[work.slottype]}</span> <span class="slot-main slot-shot">${work.shot} </span> <span class="slot-per">${work.percentage}% </span></td>`)[0];
            }else{
                cel1 = $(`<td><span class="slot-type">${slotdata[work.slottype]}</span></td>`)[0];
            }
